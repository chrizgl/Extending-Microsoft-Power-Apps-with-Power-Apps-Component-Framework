...
export class WebApiExample implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    private async OnCreateButtonClicked(): Promise<void> {
        const contactData: ComponentFramework.WebApi.Entity = {};
        contactData['firstname'] = 'WebApi';
        contactData['lastname'] = 'Testing';
        contactData['fullname'] = 'WebApi Testing';

        const resp = await this.theContext.webAPI
            .createRecord('contact', contactData)
    ...
    private async OnRetrieveButtonClicked(): Promise<void> {
        if (this.contactEntityId) {
            const con = await this.theContext.webAPI
                .retrieveRecord('contact', this.contactEntityId, '?$select=fullname,jobtitle')
    ...
    private async OnDeleteButtonClicked(): Promise<void> {
        if (this.contactEntityId) {
            const con = await this.theContext.webAPI
                .deleteRecord('contact', this.contactEntityId)
    ...
    private async OnRetrieveMultipleButtonClicked(): Promise<void> {
        const result = await this.theContext.webAPI
            .retrieveMultipleRecords('cgsol_part', "?$select=cgsol_prt_partnumber&$filter=contains(cgsol_prt_partnumber,'1234567')&$top=3")
        ...
        if (result) {
            ...
            result.entities.forEach((ele) => {
                this.outputLabel.innerHTML += `<br/>${ele.prt_partnumber}`;
            });
        }
    }
}
